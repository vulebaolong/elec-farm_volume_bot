...
export default function Symbols({ open, onOpenChange: setOpen }: TProps) {
  ...
  useEffect(() => {
    ...
    const handleSymbols = (data: TSocketRes<TSymbols>) => {
      ...
      const sortedSymbols = Object.values(data.data).sort((a, b) =>
            ...
            {symbols.map((item) => {
              ...
              return (
                <AccordionItem
                  ...
                  className={
                    isQualified
                      ? 'border border-green-500 shadow-md rounded-md'
                      : ''
                  }
                >
                  <AccordionTrigger className="flex justify-between items-center px-4 py-2 rounded-md border shadow-sm bg-muted">
                    <div className="flex items-center gap-1 text-xs font-medium">
                      ...
                      <div className="flex gap-1">
                        <Badge
                          variant={item.flags?.isLong ? 'default' : 'outline'}
                          className={
                            item.flags?.isLong ? 'bg-green-500 text-white' : ''
                          }
                        >
                          Long
                        </Badge>
                        <Badge
                          variant={item.flags?.isShort ? 'default' : 'outline'}
                          className={
                            item.flags?.isShort ? 'bg-red-500 text-white' : ''
                          }
                        >
                          Short
                        </Badge>
                        <Badge
                          variant={item.flags?.isDepth ? 'default' : 'outline'}
                        >
                          Depth
                        </Badge>
                        <Badge
                          variant={
                            item.flags?.isSpreadPercent ? 'default' : 'outline'
                          }
                        >
                          Spread
                        </Badge>
                  ...
                  <AccordionContent className="bg-background px-4 py-2 text-sm space-y-1 rounded-md">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <Badge variant={'outline'}>
                        {item.imbalanceBidPercent?.toFixed(2)}%
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-sm text-muted-foreground">
                      ...
                      <Badge variant={'outline'}>
                        {item.imbalanceAskPercent?.toFixed(2)}%
                      </Badge>
                    </div>
                  </AccordionContent>
                </AccordionItem>
