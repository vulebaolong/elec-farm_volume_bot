import { useClearAllBlackList, useCreateBlackList, useGetMyBlackList, useRemoveBlackList } from "@/api/tanstack/black-list.tanstack";
...
export default function Blacklist() {
    ...
    const { data: blacklistData } = useGetMyBlackList();
    const createBlackList = useCreateBlackList();
    const removeBlackList = useRemoveBlackList();
    const clearAllBlackList = useClearAllBlackList();

    const [q, setQ] = useState("");
    const [blacklist, setBlacklist] = useState<string[]>([]); // TODO: có thể hydrate từ server

    const allSymbols = useMemo(() => (data ?? []).map((s) => s.symbol).sort(), [data]);

    const filtered = useMemo(() => {
        const qq = q.trim().toLowerCase();
        return allSymbols.filter((s) => !blacklist.includes(s) && (qq ? s.toLowerCase().includes(qq) : true));
    }, [allSymbols, blacklist, q]);

    useEffect(() => {
        if (!blacklistData) return;
        setBlacklist(blacklistData.map((x) => x.symbol));
    }, [blacklistData]);

    const add = (sym: string) => {
        createBlackList.mutate({ symbol: sym });
        setBlacklist((prev) => (prev.includes(sym) ? prev : [...prev, sym]));
    };

    const remove = (sym: string) => {
        removeBlackList.mutate({ symbol: sym });
        setBlacklist((prev) => prev.filter((x) => x !== sym));
    };

    const clearAll = () => {
        clearAllBlackList.mutate();
        setBlacklist([]);
    };
    ...
    return (
        <div className="w-full">
            <p className="text-xl font-bold text-muted-foreground">Blacklist {blacklist.length}</p>
                ...
                <Card className="border-border p-0 gap-0 rounded-md">
                    <CardHeader className="flex items-center justify-between px-3 py-2 border-b bg-muted/50">
                        <div className="flex items-center gap-2">
                            <h3 className="text-sm font-semibold">Blacklist hiện tại</h3>
                            <Badge variant="secondary">{blacklist.length}</Badge>
                        </div>
                        <Button variant="destructive" size="sm" className="gap-1 !h-full" onClick={clearAll} disabled={blacklist.length === 0}>
                            <Trash2 className="!h-3 !w-3" />
                            <span className="!text-sm">Clear all</span>
                        </Button>
                    </CardHeader>
                    {/* <CardHeader className="p-2 gap-0 flex flex-row items-center justify-between">
                        <CardTitle className="text-base">
                            Blacklist hiện tại <Badge variant="outline">{blacklist.length}</Badge>
                        </CardTitle>
                        <Button variant="destructive" size="sm" className="gap-1 p-1" onClick={clearAll} disabled={blacklist.length === 0}>
                            <Trash2 size={5} />
                            Clear all
                        </Button>
                    </CardHeader> */}
                    ...
                    <CardContent className="p-0">
                        <ScrollArea className="h-[203px]">
                            {blacklist.length === 0 ? (
                                <div className="p-4 text-sm text-muted-foreground">Chưa có symbol nào trong blacklist.</div>
                            ) : (
            ...
            {/* TODO: Nút Lưu nếu bạn có API lưu blacklist */}
            {/* <div className="flex justify-end">
        <Button onClick={() => saveToServer(blacklist)} className="gap-2">
