...
import { useUpdateSettingUser } from "@/api/tanstack/setting-user.tanstack";
...
export default function Setting({ open, onOpenChange: setOpen }: TProps) {
    ...
    useEffect(() => {
        if (settingUser) {
            form.reset({
                maxTotalOpenPO: settingUser.maxTotalOpenPO ?? "",
                leverage: settingUser.leverage ?? "",
                inputUSDT: settingUser.inputUSDT ?? "",
                ifImbalanceBidPercent: settingUser.ifImbalanceBidPercent ?? "",
                ifImbalanceAskPercent: settingUser.ifImbalanceAskPercent ?? "",
                takeProfit: settingUser.takeProfit ?? "",
                stopLoss: settingUser.stopLoss ?? "",
                timeoutMs: settingUser.timeoutMs ?? "",
                timeoutEnabled: settingUser.timeoutEnabled ?? false,
            });
        }
    }, [settingUser, form]);
    ...
    function onSubmit(raw: FormInput) {
        ...
        updateSettingUser.mutate(payload, {
            onSuccess: (data) => {
                ...
                // queryClient.invalidateQueries({ queryKey: [`get-setting-system`] });
            ...
            <SheetContent className="w-[500px] !max-w-full gap-0">
                ...
                <SheetFooter>
                    <div className="flex w-full items-center justify-between gap-2">
                        <ButtonLoading className="flex-1" loading={updateSettingUser.isPending} type="submit" form="setting">
                            Save
                        </ButtonLoading>
