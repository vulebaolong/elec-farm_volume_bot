...
export const useWebSocketHandler = ({ webviewRef, handleOpenEntry }: TUseWebSocketHandler) => {
    ...
    }, [isStart, webviewRef, settingSystem, SettingUsers, whitelistResetInProgress, handleOpenEntry]);

    const handleEntry = useCallback(async ({ data }: TSocketRes<SymbolState[]>) => {
        // console.log({ handleEntry: data });
        
        const { isStart, webviewRef, SettingUsers, whitelistResetInProgress, handleOpenEntry } = latestRef.current;
