...
class Bot {
    ...
    private async run() {
        ...
        while (this.running) {
            ...
            try {
                this.log("\n\n\n\n\n");
                this.log(`✅✅✅✅✅ ITER START ${this.count} =====`, this.snapshot());
                this.beforeEach();
                if (this.isStart) {
                    const orderOpens = await this.getOrderOpens();
                    console.log("refreshOrderOpens: ", orderOpens);
                    // let isRefresh = true;
    ...
    private seq = 0;
    private gateFetch<T>(url: string, init?: any, timeoutMs = 5): Promise<TFectWorkRes<T>> {
        ...
        return new Promise<TFectWorkRes<T>>((resolve) => {
            const onMsg = (m: any) => {
                try {
                    ...
                    const payload: TGateFectMainRes = m.payload;
                    ...
                    resolve({ ok: true, body, error: null });

                    return;
                } catch (e) {
                    resolve({ ok: false, body: null, error: String(e) });
                    return;
                } finally {
